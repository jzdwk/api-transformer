...
local function transform_param(real_req, trans_table, conf)
	...
	-- api_fr_param = {"id1","id2"}
	-- api_fr_prefix = "cmcc"
	-- return {id1=100,id2=200}
    local api_fr_params = path_params_mgr.parse_params(conf.api_fr_path, real_req.path, conf.pathParams, conf.api_fr_prefix)	
    local api_bk_path = conf.api_bk_path  
    -- set host to nil
    headers = change_head_value(1, headers, HOST, nil)
	--是否在config.XX中设置了config.replace / config.add, 
