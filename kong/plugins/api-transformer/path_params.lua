...
-- real_path 即实际的请求uri，比如 /cmcc/prefix/req/100/info/200
-- api_fr_params 即config.api_fr_params，e.g. id1 id2
-- e.g.
-- api_fr_path = "/req/{id1}/info/{id2}"
-- real_path = "/cmcc/prefix/get/100/info/200"
-- api_fr_params = {"id1","id2"}
-- api_fr_prefix = "cmcc/prefix"
...
function _M.parse_params(api_fr_path, real_path, api_fr_params, api_fr_prefix)
	...
	for v in string.gmatch(real_path, "([^/]+)") do
		...
		--实际path的list, e.g. cmcc/prefix/req/100/info/200->{cmcc,prefix,req,100,info,200}
		table.insert(value_list, str)
	end

	-- remove path prefix {cmcc,prefix,req,100,info,200}->{req,100,info,200}
	...
	for i = 1, #api_fr_params do
		...
		local pattern = "{" .. param .. "}"
		--get index of params xxx from reversed keylist , {req=1,{id1}=2,info=3,{id2}=4}->key_list[{id1}]=2
		local pos = key_list[pattern]
		--params[id1]=100, {req,100,info,200}[2]=100
